...
double integral_over_subdomain_f(INMOST_ICE_nodes& n, INMOST::Tag& m)
{
        ...
        ++trianit)
    {
        ...
        {
            // make linear function on triangle 
            INMOST::ElementArray<Node> local_nodes = trianit->getNodes();

            double lon0 = local_nodes[0]->RealArray(n.GetCoords().geo_coords)[0];
            double lat0 = local_nodes[0]->RealArray(n.GetCoords().geo_coords)[1];

            double lon1 = local_nodes[1]->RealArray(n.GetCoords().geo_coords)[0];
            double lat1 = local_nodes[1]->RealArray(n.GetCoords().geo_coords)[1];

            double lon2 = local_nodes[2]->RealArray(n.GetCoords().geo_coords)[0];
            double lat2 = local_nodes[2]->RealArray(n.GetCoords().geo_coords)[1];

            std::vector<std::vector<double>> local_node_coords = 
            {{lon0, lat0}, {lon1, lat1}, {lon2, lat2}};

            local_node_coords = GeoTrCorr(local_node_coords);
            ...
            res += integral_over_triangle(GeoTrCorr(local_node_coords), f);
        }
    }
    return res;
}
...
double integral_over_subdomain_squared_f1_minus_f2(INMOST_ICE_nodes& n, INMOST::Tag& m1, INMOST::Tag& m2)
{
        ...
        ++trianit)
    {
        if (trianit->GetStatus() != Element::Ghost)
        {
            ...
            // calculate integral over triangle and add to res
            res += integral_over_triangle(GeoTrCorr(local_node_coords), f*f);
        }
    }
    return res;
}
