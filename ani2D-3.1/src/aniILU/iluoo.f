...
c
          ...
          if (partlur .le. 0) then
             call errMesg(1000, 'iluoo', 'non-positive partur')
          end if
          if (partlur .ge. 1) then
             call errMesg(1000, 'iluoo', 'partlur too big')
          end if
     ...
     &        lenlu, lenr, lenluout, lenrout, ierr)
          if (ierr .eq. 1) then
            if (lendwork - 3 * n .lt. leniwork - 4 * n - 2) then
               call errMesg(1000, 'iluoo', 
     &                      'lendwork too small or partlur too big')
             else   
               call errMesg(1000, 'iluoo', 
     &                      'leniwork too small or partlur too big')
            end if
          end if
          if (ierr .eq. 2) then
            if (lendwork - 3 * n .lt. leniwork - 4 * n - 2) then
               call errMesg(1000, 'iluoo', 
     &                      'lendwork too small or partlur too small')
             else   
               call errMesg(1000, 'iluoo', 
     &                      'leniwork too small or partlur too small')
            end if
          end if
...
c        
        if (n .le. 0) then
          call errMesg(1000, 'iluoo_init', 'sflu_init: non-positive n')
        end if
c  
        do k=1,n
          mi(k) = inf
...
c
        do k=1,n 
          nzra = xa(k+1) - xa(k)
          if (nzra .eq. 0) then
            call errMesg(1000, 'iluoo_init', 'sflu_init: empty row')
c           print *, 'sflu_init: empty row at number ', k
          endif
...
c        
        if (n .le. 0) then
           call errMesg(1000, 'genebs', 'n is non-positive')
        end if
        if (niter .le. 0) then
           call errMesg(1000, 'genebs', 'niter is non-positive')
        end if
c
        do k=1,n
         u(k) = 0d0
...
c  initial guess = e
        ...
        if (scmin .lt. eps) then
           call errMesg(1000, 'genebs', 'almost zero matrix row')
c          write (*,*) 
c    &       'genebs: (almost) zero matrix row, squared length =', scmin
        end if
        do k=1,n
           u(k) = 1d0/u(k)
        end do
